...
class SSNE:
	...
	def crossover_inplace(self, gene1, gene2):
		...
		gen1 = np.array(self.gene1.get_weights())
		gen2 = np.array(self.gene2.get_weights())
	...
	def mutate_inplace(self, gene):
		...
		num_params = model_params.shape[0]*model_params.shape[1]*model_params[0].shape[0]
		...
		for layer in model_params:
			for W in layer:
				if len(W.shape) == 2: #Weights, no bias, the original implementation doesn't mutate the bias matrix
	...
	def clone(self, master, replacee):  # Replace the replacee individual with master
		replacee.set_weights(master.get_weights())
